...
bool HelloWorld::createBox2dWorld(bool debug) {
	...
	this->addNewSpriteAtPosition(Vec2(winSize.width/2, winSize.height/2));
	return true;
}
...
void HelloWorld::tick(float dt) {
	...
	/*
	if(playerIsFlying){
		body->ApplyLinearImpulse(b2Vec2(0, playerVelocity), body->GetWorldCenter(), true);
...
bool HelloWorld::onTouchBegan(Touch* touch, Event* evnet) {
	...
	playerIsFlying = true;
	*/
	this->schedule(schedule_selector(HelloWorld::AddForce));
	return true;
}
void HelloWorld::onTouchEnded(Touch* touch, Event* evnet) {

	this->unschedule(schedule_selector(HelloWorld::AddForce));
	/*
	playerVelocity = 0.0f;
...
void HelloWorld::AddForce(float t) {
	body->ApplyForce(b2Vec2(0, 50), b2Vec2(body->GetPosition().x, body->GetPosition().y), true);
}